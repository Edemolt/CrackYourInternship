...
class Solution {
  public:
    ...
    Node* Bst(int pre[], int size) {
        ...
        // Create a stack to hold nodes and their corresponding valid range
        stack<Node*> s;
        s.push(root);
        ...
        for (int i = 1; i < size; i++) {
            ...
            // Keep popping from the stack while the next value is greater than the stack's top
            while (!s.empty() && pre[i] > s.top()->data) {
                temp = s.top();
                s.pop();
            }

            // If the current value is less than the last popped node,
            // it should be in the right subtree of that node.
            if (temp != NULL) {
                temp->right = newNode(pre[i]);
                s.push(temp->right);
            }
            // Otherwise, it's in the left subtree of the stack's top node.
            else {
                temp = s.top();
